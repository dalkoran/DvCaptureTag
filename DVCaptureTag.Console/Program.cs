namespace DVCaptureTag.Console
{
    ...
    class Program
    {
        ...
        {
            ////const string path = @"\\LodaxServer\Multimedia\Video\Raw\DCR-PC100E\AUS2000_1\AUS2000_1 - Clip 142.avi";
            ////const string path = @"H:\DVMetaTest\AUS2000_1 - Clip 139.avi";
            ////const string folder = @"H:\DVMetaTest\SCRATCH_1";
        ...
        private static DVMetadata GetDVMetadata(string path, Options options)
        {
            ...
            var mediaInfo = new MediaInfoWrapper(path, null);
            var filePtr = new MediaInfo(path);
            filePtr.Open(path);
        ...
        private static void UpdateFileCreationTime(string path, DVMetadata metadata, Options options, Stats stats)
        {
            if (metadata.RecordedDate.HasValue)
            {
                ...
                if (osFile.CreationTime != metadata.RecordedDate)
                {
                    ...
                    if (!options.Quiet)
                    {
                        Console.WriteLine($"  CreationTime updated to match Recorded Date: {metadata.RecordedDate.Value}");
                    }
                }
            }
        }
